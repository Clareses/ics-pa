...
int parse_elf_file(const char* name) {
    ...
    for (size_t i = 1; i < elf_header->e_shnum; i++) {
        Elf64_Shdr* sec = sec_header + i;
        if (!strcmp(sec_str_tab + sec->sh_name, ".strtab") &&
            sec->sh_type == SHT_STRTAB) {
            str_tab = mem + sec->sh_offset;
        }
        if (!strcmp(sec_str_tab + sec->sh_name, ".symtab") &&
    ...
    Elf64_Sym* sym_tab_item = (Elf64_Sym*)sym_tab;
    Elf64_Sym* sym_tab_end = (Elf64_Sym*)sym_tab + sym_tab_size;
    while (sym_tab_item < sym_tab_end) {
        if (ELF64_ST_TYPE(sym_tab_item->st_info) == STT_FUNC) {
            FuncDataItem* func = &func_symbol_table[func_data_list_length++];
            func->addr = sym_tab_item->st_value;
            strncpy(func->func_name, (char*)(str_tab + sym_tab_item->st_name), 63);
        }
        sym_tab_item++;
    }
